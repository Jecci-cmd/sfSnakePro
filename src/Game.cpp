...
// 地图大小设置
const int Game::mapWidth = 2000;    
const int Game::mapHeight = 1600;   
...
sf::VideoMode Game::initVideoMode_() // 初始化GlobalVideoMode
{
    return sf::VideoMode(1000, 800, 32); 
}

sf::VideoMode Game::GlobalVideoMode = Game::initVideoMode_(); // 初始化窗口大小
sf::View view(sf::FloatRect(0, 0, 1000, 800));  // 视口大小和窗口一样
...
std::shared_ptr<Screen> Game::MainScreen = std::make_shared<MenuScreen>();//控制当前页面的
...
{
    if (!Game::GlobalFont.loadFromFile("assets/fonts/SourceHanSansSC-Bold.otf")) {
    std::cerr << "Error loading font" << std::endl;
}
    window_.create(
        GlobalVideoMode,   // videoMode对象，决定窗口宽高和颜色深度
...
{
    static sf::Event event; // 创建一个 静态事件对象，意味着这个 event 在多次函数调用之间只创建一次（省内存，但不是必须）。

    while (window_.pollEvent(event)) // 玩家点击关闭按钮
    {
        if (event.type == sf::Event::Closed)
            window_.close();
    }

    if (window_.hasFocus() && // 窗口有焦点，鼠标在游戏窗口内，交给MainScreen::handleInput处理
        sf::FloatRect(
            sf::Vector2f(0, 0),
            sf::Vector2f(
...
{   auto gameScreen = std::dynamic_pointer_cast<sfSnake::GameScreen>(Game::MainScreen);
    if (gameScreen) {
        sf::Vector2f center = gameScreen->snake_.getHeadPosition();
    
        // 限制视图中心不超界
        float halfViewWidth = 1000 / 2.f;
        float halfViewHeight = 800 / 2.f;
    
        if (center.x < halfViewWidth)
            center.x = halfViewWidth;
        else if (center.x > mapWidth - halfViewWidth)
            center.x = mapWidth - halfViewWidth;
    
        if (center.y < halfViewHeight)
            center.y = halfViewHeight;
        else if (center.y > mapHeight - halfViewHeight)
            center.y = mapHeight - halfViewHeight;
